...
// .append(MainActivity.prompt).append("\n\n")
...
// user 메시지도 추가
        ...
        client.newCall(request).enqueue(new Callback() {
            ...
            public void onFailure(Call call, IOException e) {
                ...
                e.printStackTrace(); // 자세한 에러 원인 로그
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                if (!response.isSuccessful()) {
                    ...
                    System.err.println("GPT 응답 실패: HTTP " + response.code());

                    if (response.body() != null) {

                        String errorBody = response.body().string();

                        System.err.println("응답 본문 (에러 메시지): " + errorBody);
...
// 에러 메시지가 포함되어 있는지 검사 (OpenAI 에러는 일반적으로 에러 필드 포함)

                if (resultJson.has("error")) {

                    JsonObject errorObj = resultJson.getAsJsonObject("error");

                    System.err.println("OpenAI API 에러:");

                    System.err.println(" 코드: " + errorObj.get("code"));

                    System.err.println(" 메시지: " + errorObj.get("message"));

                    System.err.println(" 타입: " + errorObj.get("type"));
